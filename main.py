...
from model.models import SetModel, SetScoreModel, SetTTLModel
...
@app.post("/set_score")
async def set_score(
    body: SetScoreModel, redis: aioredis.Redis = Depends(get_redis)
):
    ...
    if new_score is not None:
        ...
        return {
            "message": f"사용자 '{body.user_id}'의 점수 기록에 실패했습니다."
        }


@app.get("/get_ranker/{n}")
async def get_ranker(
    n: int, redis: aioredis.Redis = Depends(get_redis)
):
    # 상위 N명의 사용자 점수 조회
    top_users = await redis.zrevrange('score_board', 0, n-1, withscores=True)
    
    # 사용자와 점수를 보기 쉽게 변환
    rank_list = [{"user_id": user.encode("utf-8"), "score": score} for user, score in top_users]
    
    return {
        "message": f"상위 {n}명의 사용자와 점수입니다.",
        "rankers": rank_list
    }
...
async def get_ranker(
    userId: str, redis: aioredis.Redis = Depends(get_redis)
):
    # 상위 N명의 사용자 점수 조회
    ...
    return {
        "message": f"사용자 '{userId}'의 순위는 {rank + 1}, 점수는 {score}입니다."
    }